...
struct behavior_mouse_move_data {
...
struct behavior_mouse_move_config {
    int16_t x_code;
    int16_t y_code;
    int delay_ms;
...
static float speed(const struct behavior_mouse_move_config *config, float max_speed,
                   int64_t duration_ms) {
...
static float update_movement_1d(const struct behavior_mouse_move_config *config,
...
static struct vector2d update_movement_2d(const struct behavior_mouse_move_config *config,
...
static bool should_be_working(struct behavior_mouse_move_data *data) {
    return is_non_zero_2d_movement(&data->state);
}

static void tick_work_cb(struct k_work *work) {
    struct k_work_delayable *d_work = k_work_delayable_from_work(work);
    struct behavior_mouse_move_data *data =
        CONTAINER_OF(d_work, struct behavior_mouse_move_data, tick_work);
    const struct device *dev = data->dev;
    const struct behavior_mouse_move_config *cfg = dev->config;
...
static void update_work_scheduling(const struct device *dev) {
    struct behavior_mouse_move_data *data = dev->data;
...
static int behavior_mouse_move_init(const struct device *dev) {
    struct behavior_mouse_move_data *data = dev->data;
...
#define KP_INST(n)                                                                                 \
    static struct behavior_mouse_move_data behavior_mouse_move_data_##n = {};                      \
    static struct behavior_mouse_move_config behavior_mouse_move_config_##n = {                    \
    ...
    DEVICE_DT_INST_DEFINE(n, behavior_mouse_move_init, NULL, &behavior_mouse_move_data_##n,        \
